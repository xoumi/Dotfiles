...
CONFIG_FILE="${HOME}/.config/todo/todofi"
...
MARKUP_CONTEXT='<span fgcolor="red"><b>\1<\/b><\/span>'
...
runrofi () {
    $ROFI_BIN -width 100% -matching glob -tokenize -i -no-levenshtein-sort -theme "$DOT_DIR/rofi/todofi.rasi" "$@"
}
    ...
    do
        ...
        selection=$( \
            ...
            runrofi `linescount $count` -kb-custom-1 "${SHORTCUT_NEW}" \
                                        ...
                                        -markup-rows \
                                        -u "${high}" -a "${medium}" -p "${current_filter}" -dmenu)
        val=$?
